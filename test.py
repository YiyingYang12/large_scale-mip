...
class LitMipNeRF360(LitModel):
    def __init__(
        ...
        # Load the text from the test.csv file using csv reader
        # Read the text data from test.csv
        with open('/root/NeRF-Factory-main/NeRF-Factory-main/test.csv', 'r') as csvfile:
            ...
            with torch.no_grad(): 
                sentence_embeddings = text_mode.encode_text(text_tokens)
                text_embeddings = sentence_embeddings.unsqueeze(0).float()
        
        print("text_embedding:",text_embeddings.shape)
        text_embeddings = text_embeddings.reshape(388,256)
